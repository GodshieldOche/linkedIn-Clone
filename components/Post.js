...
import { setModalState, setModalType } from "../redux/features/modal";
import { setPostState } from "../redux/features/post";
import { postDeletePost } from "../redux/features/posts";




const Post = ({post, modalPost, index}) => {
    ...
    const deletePost = (postId, index) => {
      dispatch(postDeletePost({postId, index}))
    };
    ...
    return (
        ...
        >
            <div className="flex items-center px-2.5 cursor-pointer">
                ...
                {modalPost ? (
                    <IconButton onClick={() => dispatch(setModalState(false))}>
            ...
            <div className="flex justify-evenly items-center dark:border-t border-gray-600/80 mx-2.5 pt-2 text-black/60 dark:text-white/75">
                ...
                {session?.user?.email === post.email ? (
                    ...
                    >
                        <DeleteRoundedIcon />
                        <h4>{loading ? "loading" : "Delete post"}</h4>
                    </button>
                ) : (
                    <button className="postButton ">
